C     Last change:  RLC  23 Nov 96   12:37 pm
*+
      PROGRAM NACA6
*   --------------------------------------------------------------------
...
*
*     AUTHORS - Charles L. Ladson and Cuyler W. Brooks, Jr.
...
*   Sep74  1.0 CLL&CWB Original coding
*   Nov89  1.1   LH    Interpolation routine FTLUP replaced by LCSFIT to
*                        eliminate non-standard %LOC usage; finite
*                        difference derivatives (FUNCTION DIF) replaced
*                        with FDI2K; assumed SINH and COSH are standard
*                        library functions
*  1Dec94  1.2   RLC   Replaced FTLUP and FDI2K with spline fit
* 13Dec94  1.3   RLC   Named the output files
* 29Apr95  1.4   RLC   Renamed the Namelist to INPUT6 to avoid confusion
*                         with the NACA4 program
*  3May95  1.5   RLC   Made a file that can be plotted with gnuplot
*  8Aug95  1.6   RLC   Several mods to the output format
* 11Aug95  1.7   RLC   Made the names PROFILE and CAMBER to agree with NACA4
* 14Aug95  1.8   RLC   Added CAMBER='NONE'
*  6Nov95  1.9   RLC   Tells you if there is no good data
* 23Nov96  2.0   RLC   Adds declaration of I
*
*     NOTES- This program has also been known as LADSON and SIXSERIES
*        and as SIXSERIE on systems with a 8-letter name limit
*
...
*     C O N S T A N T S                                                *
************************************************************************
      INTEGER LUNRD,LUNOUT,LUNSAV,LUNFIG ! units
      PARAMETER (LUNRD=1, LUNOUT=2, LUNFIG=3, LUNSAV=10)
      INTEGER MXCOMB
      INTEGER NX
      PARAMETER (MXCOMB=10, NX=200)

      CHARACTER GREETING*60, AUTHOR*60, VERSION*30, FAREWELL*60
      CHARACTER MODIFIER*60
      PARAMETER (GREETING='naca6 - compute airfoil coordinates')
      PARAMETER (AUTHOR=
     &     'Charles L. Ladson and Cuyler W. Brooks, Jr., NASA Langley')
      PARAMETER (MODIFIER=' ')   ! put your name here if you make mods
      PARAMETER (VERSION='1.9 (6Nov95)' )
      PARAMETER (FAREWELL=
     &   ' Files naca6.out and naca6.plt added to your directory.')
      REAL PI
...
************************************************************************
      ...
      REAL CMBNMR
      REAL COSTH,SINTH
      REAL dummy   ! argument to Seval3 for values we don't really need
      REAL DX
      REAL E
      INTEGER errCode
      REAL F
...
*     A R R A Y S                                                      *
************************************************************************
      REAL YUPR(NX), YLPR(NX),
     >   XU(NX), XL(NX), YU(NX), YL(NX),
     >   XAU(NX), YAU(NX), XAL(NX), YAL(NX)
      REAL XT(NX+1), YT(NX+1), YTP(NX+1), YTPP(NX+1), YTPPP(NX+1),
     >   PHI(NX+1), EPS(NX+1), PSI(NX+1)
      REAL CLI(MXCOMB), A(MXCOMB), TANTH0(MXCOMB), YCMB(MXCOMB),
     >   TANTH(MXCOMB), YCP2(MXCOMB)
      REAL AYP(NX+1), AYPP(NX+1), AX(NX+1), AXUC(NX+1), AYUC(NX+1),
     >   AYUP(NX+1), AXLC(NX+1), AYLC(NX+1), AYLP(NX+1)
      REAL bspline(NX+1), cspline(NX+1), dspline(NX+1)
************************************************************************
*     N A M E L I S T   D E F I N I T I O N S                          *
************************************************************************
      NAMELIST /INPUT6/ A, CHD, CLI, CMBNMR, NAME, CAMBER, PROFILE, TOC
*
C  NAMELIST "INPUT6":
C
C   VAR     DIM   TYPE    DESCRIPTION
C  NAME      -    C*80    Title desired on printed and plotted output
C  PROFILE   -    C*3     NACA airfoil family (series 63 thru 67 and
C                         63A thru 65A)
C  CAMBER    -    C*3     Camber line (series 63 thru 66 and 63A thru 65A)
...
*
  ...
  100 CONTINUE                             ! not really needed anymore
*..... If you use Watcom Fortran, change NML to FMT on following statement
      READ(LUNRD, NML=INPUT6, END=999 )
      ICKY=INT(CMBNMR)   ! convert to integer. CMBNMR never used again
      IF(ICKY.LT.1) ICKY=1
      DO 110 J=ICKY+1,10
         CLI(J)=0.0
         A(J)=0.0
  110 CONTINUE
      ...
      FRAC=1.0
*
*..... Set up the files for printing and plotting ......................
*      You may want to delete the plot header line (depends on how you plot)
      OPEN(UNIT=LUNOUT, FILE='naca6.out', STATUS='UNKNOWN')
      OPEN(UNIT=LUNFIG, FILE='naca6.plt', STATUS='UNKNOWN')
      WRITE(LUNFIG,*) '#Created by NACA6 from '//fileName
*

*     Print all input data (including defaults) ........................
      WRITE(LUNOUT,*) 'NACA6 - A PROGRAM TO COMPUTE NACA '//
     & '6-SERIES AND 6A-SERIES AIRFOIL SECTIONS'
      WRITE(LUNOUT,*)
      WRITE(LUNOUT,*) 'SUMMARY OF INPUT DATA AND DEFAULTS'
      WRITE(LUNOUT,*)
      WRITE(LUNOUT, NML=INPUT6)                  ! prints all input data
      WRITE(LUNOUT,5000)   !--------------------------------------------
*
      IF (CHD .NE. 1.0) THEN
     ...
     &   '    an airfoil will be printed with chord length = ', CHD
      ...
      IF (CAMBER.EQ.'NONE') THEN                   ! added 14Aug95   RLC
        WRITE(LUNOUT,*) 'Uncambered airfoil'
        A(1)=0.0
        CLI(1)=0.0
        CMBNMR=1.0
      ELSEIF (CAMBER.EQ.'63'.OR.CAMBER.EQ.'64'.OR.CAMBER.EQ.'65'.OR.
     &        CAMBER.EQ.'66'.OR.CAMBER.EQ.'67' ) THEN
        WRITE(LUNOUT,*) '      Standard 6-series camber line'
      ELSEIF (CAMBER.EQ.'63A'.OR.CAMBER.EQ.'64A'.OR.CAMBER.EQ.'65A')THEN
        WRITE(LUNOUT,*) '      Modified 6-series camber line'
      ELSE
...
*
  200 CONTINUE
     ...
     >             (A(1)+1.)/2.0
         IF(L.LT.ICKY.AND.ICKY.GT.1) GO TO 200
...
C     Start loop for X increment:
  300 CONTINUE    ! keep coming back here
C       Skip thickness computation after first pass:
      IF(I.GT.1) GO TO 550
C       Select series:
      IF (PROFILE.EQ.'63')  CALL PHEP63  (PHI,EPS)
      IF (PROFILE.EQ.'64')  CALL PHEP64  (PHI,EPS)
      IF (PROFILE.EQ.'65')  CALL PHEP65  (PHI,EPS)
      IF (PROFILE.EQ.'66')  CALL PHEP66  (PHI,EPS)
      IF (PROFILE.EQ.'67')  CALL PHEP67  (PHI,EPS)
      IF (PROFILE.EQ.'63')  CALL PHPS63  (PHI,PSI)
      IF (PROFILE.EQ.'64')  CALL PHPS64  (PHI,PSI)
      IF (PROFILE.EQ.'65')  CALL PHPS65  (PHI,PSI)
      IF (PROFILE.EQ.'66')  CALL PHPS66  (PHI,PSI)
      IF (PROFILE.EQ.'67')  CALL PHPS67  (PHI,PSI)
      IF (PROFILE.EQ.'63A') CALL PHEP63A (PHI,EPS)
      IF (PROFILE.EQ.'64A') CALL PHEP64A (PHI,EPS)
      IF (PROFILE.EQ.'65A') CALL PHEP65A (PHI,EPS)
      IF (PROFILE.EQ.'63A') CALL PHPS63A (PHI,PSI)
      IF (PROFILE.EQ.'64A') CALL PHPS64A (PHI,PSI)
      IF (PROFILE.EQ.'65A') CALL PHPS65A (PHI,PSI)
      RAT=1.0
      IT=0
      ACRAT=1.0

      WRITE(LUNOUT,*) 'ITERATION TO ACHIEVE SPECIFIED THICKNESS'
C       Loop start for thickness iteration:
  400 CONTINUE                           ! was 150
         IT=IT+1
         WRITE(LUNOUT,1003)IT,RAT
         ACRAT=ACRAT*RAT
         YMAX=0.0
         DO 410 J=1,NX+1   ! changed by RLC from NX  1Dec94
...
!.... Estimate first,second and third derivatives by spline fit.........
      CALL Spline(NX+1, XT,YT, bspline,cspline,dspline)
      DO j=1,NX+1
         CALL Seval3(NX+1, XT(J), XT,YT, bspline,cspline,dspline,
     &      dummy, YTP(J), YTPP(J), YTPPP(J) )
      ENDDO
*
*
...
!!!         CALL LCSFIT (NX+1, XT, YT, .TRUE., 'B', 1, XTP, YM, YM)
         CALL Seval3(NX+1, XTP, XT,YT, bspline,cspline,dspline,
     &      YM, dummy,dummy,dummy)
         XO=XT(1)
         XL(1)=XT(NX)
         TR=2.*YM/(XL(1)-XO)
         RAT=TOC/TR
     ...
     >GO TO 400                           ! keep going back until TR=TOC
...
C        Fit tilted ellipse at eleventh profile point:
  ...
  510    CONTINUE
         RNP=BN**2/AN
         IF (I.EQ.1) THEN
           WRITE(LUNOUT,1004)XYM,YMAX,XTP,YM,XT(11),YT(11),
     >               YTP(11),RNP,RAT,ACRAT,IT
           WRITE(LUNOUT,5000)   !---------------------------------------
         ENDIF
         CALL Spline(NX+1, XT,YT, bspline,cspline,dspline) ! update it
      END IF
*
...
!!!!        WRITE(LUNOUT,1002)
     ...
     &             'XL/C', 'YL/C', 'dYL/dXL'
      ...
      X=0.00025
      XL(1)=0.0
  550 CONTINUE   ! jump to here from above when I > 1
      YUPR(I) = YUP
      YLPR(I) = YLP
...
!!!      CALL LCSFIT (NX, XT, YTPP, NEW, METHOD, NEVAL, X, YPP, YPP)
      CALL Spline(NX+1, XT,YT,  bspline,cspline,dspline)
      CALL Seval3(NX+1, X, XT,YT,
     &                      bspline,cspline,dspline,Y,YP,YPP,dummy)
...
C        Modified camberline option:
         IF(CAMBER.EQ.'63A' .OR. CAMBER.EQ.'64A' .OR. CAMBER.EQ.'65A')
     >      THEN
...
C     Combine thickness distributuion and camberline:
      ...
      YL(I)=YCMB(1)-Y*COSTH
      IF (X.GE. .815) THEN
         IF(PROFILE.EQ.'63A' .OR. PROFILE.EQ.'64A' 
     &                       .OR. PROFILE.EQ.'65A') THEN
            IF (X.LE. .825) THEN
               X2 = 1.0
               X1 = XU(I)
               Y2 = 0.
...
*..... Store scaled profile in appropriate arrays ......................
      AXUC(I) = XUC
      AYUC(I) = YUC
      IF (ALI.GE.TINY .OR. ICKY.NE.1) THEN 
         AX(I) = X                         
         AXLC(I) = XLC
         AYLC(I) = YLC
      END IF
...
C     Increment X and return to start of X loop:
      ...
      XAL(I) = XLC
      YAL(I) = YLC
      IF(X.LE.1.0) GO TO 300   ! keep going back to 300 until x>1

C     Calculate derivatives of final coordinates; tabulate and save results.

      IF (ALI.GE.E .OR. ICKY.NE.1) THEN
         CALL Spline(I, XU,YU, bspline,cspline,dspline)
         DO j=1,I
            CALL Seval3(I, XU(J), XU,YU, bspline,cspline,dspline,
     &        dummy, AYUP(j), dummy, dummy)
         ENDDO
         CALL Spline(I, XL,YL, bspline,cspline,dspline)
         DO j=1,I
            CALL Seval3(I, XL(J), XL,YL, bspline,cspline,dspline,
     &        dummy, AYLP(j), dummy, dummy)
         ENDDO
*.....     output for cambered airfoil
         WRITE (LUNOUT,1005) (AX(M), XU(M), YU(M), AYUP(M),
     &                               XL(M), YL(M), AYLP(M), M = 1, I)
      ELSE
         CALL Spline(I, XU,YU, bspline,cspline,dspline)
         DO j=1,I
            CALL Seval3(I, XU(J), XU,YU, bspline,cspline,dspline,
     &        dummy, AYP(j), AYPP(j), dummy)
         ENDDO
*          uncambered
         WRITE (LUNOUT,1006) (XU(M),YU(M),AYP(M),AYPP(M), M = 1, I)
      END IF
*
*..... Print the scaled airfoil if CHD <> 1.0  ..........................
      WRITE(LUNOUT,5000)   !--------------------------------------------
      IF (CHD .NE. 1.0) THEN
        WRITE(LUNOUT, '(4X,A,F9.4)')
     &    'AIRFOIL SCALED TO CHORD LENGTH=', CHD
        IF (ALI.GT.TINY .OR. ICKY.GT.1) THEN
          WRITE(LUNOUT,*) '    x-upper    y-upper    x-lower    y-lower'
          WRITE(LUNOUT, '(4F11.5)' )
     &                         (AXUC(J),AYUC(J), AXLC(J),AYLC(J), J=1,I)
        ELSE
          WRITE(LUNOUT,*) '      x          y'
          WRITE(LUNOUT, '(2F11.5)') (AXUC(J),AYUC(J), J=1,I)
        ENDIF
        WRITE(LUNOUT,5000)   !------------------------------------------
      ENDIF
*
*..... Plot
      ...
      CLOSE(UNIT=LUNFIG)
      WRITE(*, '(A)' ) FAREWELL
      STOP 'NACA6 has terminated successfully.'

  999 WRITE(*,*) 'Did not find a valid namelist record.'
      WRITE(*,*) 'No output produced.'
      STOP 'No valid input.'

C     Formats:

 1001 FORMAT(//8X, 'X/C', 9X, 'Y/C', 7X, 'DY/DX', 7X, 'D2Y/DX2')
 1002 FORMAT(//7X,'MEAN', 16X, 'UPPER SURFACE', 20X, 'LOWER SURFACE'/
     >        6X, 'X/C',8X, 'XU/C', 7X, 'YU/C', 5X, 'DYU/DXU',
     &                  6X, 'XL/C', 7X, 'YL/C', 6X, 'DYL/DXL')
 ...
 5000 FORMAT('-----------------------------------------------------'///)
      END ! ------------------------------------ End of main program NACA6


*+
      SUBROUTINE PHEP63(PHI,EPS)
*   --------------------------------------------------------------------
*     PURPOSE - Compute phi, eps vectors for 63 series airfoil
*
      IMPLICIT NONE
************************************************************************
*     A R G U M E N T S                                                *
...
*     L O C A L   A R R A Y S                                          *
************************************************************************)
      REAL PHILD(201),PSILD(201)
      REAL B(201),C(201),D(201)
      DATA (PHILD(I),PSILD(I),I=1,25)/
     ...
     1    .11529,   1.53940,    .11428,   1.55510,    .11324/
      DATA (PHILD(I),PSILD(I),I=101,125)/
     ...
     1    .08365,   1.93209,    .08217,   1.94780,    .08068/
      DATA (PHILD(I),PSILD(I),I=126,150)/
     ...
     1    .04512,   2.32477,    .04362,   2.34048,    .04213/
      DATA (PHILD(I),PSILD(I),I=151,175)/
     ...
     1    .01331,   2.71745,    .01239,   2.73316,    .01149/
      DATA (PHILD(I),PSILD(I),I=176,201)/
     ...
     1   3.14159,   0.00000/
*-----------------------------------------------------------------------
      CALL SPLINE(201, PHILD, PSILD, B,C,D)                   !OK
*
      DO 202 i=1,201
         CALL Seval3(201,phi(i), phild,psild, b,c,d, psi(i), xx,xx,xx)
!!!      CALL LCSFIT (201, PHILD, PSILD, .TRUE., 'B', 201, PHI, PSI, PSI)
  202 CONTINUE 
      END ! ----------------------------------- End of subroutine PHPS63
*+
...
*
      DO 202 J=1,201
         CALL Seval3(201,phi(j), phild,psild, b,c,d, psi(j), xx,xx,xx)
!!!      CALL LCSFIT (201, PHILD, PSILD, .TRUE., 'B', 201, PHI, PSI, PSI)
  202 CONTINUE
      END ! ----------------------------------- End of subroutine PHPS64
*+
...
*     L O C A L   A R R A Y S                                          *
************************************************************************
      REAL PHILD(251),EPSLD(251)
      REAL B(251),C(251),D(251)
      DATA (PHILD(I),EPSLD(I),I=1,25) /
     ...
     1    .08555,   1.88120,    .08599,   1.89194,    .08641/
      DATA (PHILD(I),EPSLD(I),I=176,200) /
     ...
     1    .09052,   2.15554,    .09043,   2.16679,    .09032/
      DATA (PHILD(I),EPSLD(I),I=201,225) /
     ...
     1    .07872,   2.46440,    .07772,   2.47851,    .07666/
      DATA (PHILD(I),EPSLD(I),I=226,251) /
     ...
     1   3.14159,   0.00000/
*-----------------------------------------------------------------------
      CALL SPLINE(251, PHILD, EPSLD, B,C,D)          !OK
*
      DO 201 J=1,201
         PHI(J)=FLOAT(J-1)*3.141592654/200.0
         CALL Seval3(251,phi(j), phild,epsld, b,c,d, eps(j), xx,xx,xx)
!!!  201 CALL LCSFIT (251, PHILD, EPSLD, .TRUE., 'B', 201, PHI, EPS, EPS)
  201 CONTINUE
      END ! ----------------------------------- End of subroutine PHEP65
*+
...
************************************************************************
     ...
     1    .12307,   1.88122,    .12194,   1.89197,    .12079/
      DATA (PHILD(I),PSILD(I),I=176,200) /
     ...
     1    .09094,   2.15554,    .08953,   2.16679,    .08811/
      DATA (PHILD(I),PSILD(I),I=201,225) /
     ...
     1    .05199,   2.46431,    .05024,   2.47839,    .04850/
      DATA (PHILD(I),PSILD(I),I=226,251) /
     ...
     1   3.14159,   0.00000/
*-----------------------------------------------------------------------
      CALL SPLINE(251, PHILD, PSILD, B,C,D)              !OK
*
      DO 201 J=1,201
         CALL Seval3(251,phi(j), phild,psild, b,c,d, psi(j), xx,xx,xx)
!!!      CALL LCSFIT (251, PHILD, PSILD, .TRUE., 'B', 201, PHI, PSI, PSI)
  201 CONTINUE
      END ! ----------------------------------- End of subroutine PHPS65
*+
...
*     A R G U M E N T S                                                *
************************************************************************
      INTEGER n    ! # of data points (n>1)
      REAL    x(n) ! abscissas of knots
      REAL    y(n) ! ordinates of knots
      REAL    b(n) ! linear coeff
      REAL    c(n) ! quadratic coeff.
      REAL    d(n) ! cubic coeff.
************************************************************************
*     L O C A L   V A R I A B L E S                                    *
************************************************************************
      INTEGER i
      REAL t
*-----------------------------------------------------------------------
      IF (n .LT. 3) THEN   ! special treatment for n < 3
         b(1)=0.0
...
*   1977   1.0   FMM   Original publication
*   1981   1.1   RLC   Fortran 77 constructions; IMPLICIT NONE
* 18Apr91  1.2   RLC   Fortran 90 style loops
*
*     NOTES- From "Computer Methods for Mathematical Computations", by
*                    Forsythe, Malcolm, and Moser. Prentice-Hall, 1977.
*     If u<x(1), i=1 is used;if u>x(n), i=n is used
      IMPLICIT NONE
************************************************************************
*     A R G U M E N T S                                                *
************************************************************************
      INTEGER n ! # of data points (n>1)
      REAL u ! abscissa at which the spline is to be evaluated
      REAL x(n) ! abscissas of knots
      REAL y(n) ! ordinates of knots
      REAL b(n) ! linear coeff
      REAL c(n) ! quadratic coeff.
...
*-----------------------------------------------------------------------
      IF (n .LE. 1) THEN
         seval=y(1)
         RETURN                            ! notice the premature return
      ENDIF
...
*
   20 k=(i+j)/2
      ...
      IF (j .GT. i+1) GOTO 20
*
      dx=u-x(i)                                        ! evaluate spline
      seval=y(i)+dx*(b(i)+dx*(c(i)+dx*d(i)))
      END   ! ------------------------------------ End of function SEVAL
*+
      SUBROUTINE Seval2(n, u, x,y, b,c,d, seval,deriv)
*   --------------------------------------------------------------------
*     PURPOSE - Evaluate the cubic spline function and its derivative
*        seval=y(i)+b(i)*(u-x(i))+c(i)*(u-x(i))**2+d(i)*(u-x(i))**3
*           where  x(i)=<u<x(i+1)
*
*     AUTHORS - Forsythe, Malcolm, and Moser
...
*
      dx=u-x(i)                                        ! evaluate spline
      seval=y(i)+dx*(b(i)+dx*(c(i)+dx*d(i)))
      deriv=b(i)+dx*(c(i)+c(i)+dx*(d(i)+d(i)+d(i)))
      END   ! --------------------------------- End of subroutine SEVAL2
*+
      SUBROUTINE Seval3(n, u, x,y, b,c,d, seval,deriv,deriv2,deriv3)
*   --------------------------------------------------------------------
...
*.....I has been determined. Evaluate spline and derivatives............
      ...
      seval=y(i) + dx*(b(i) + dx*(c(i) + dx*d(i)))
      deriv=b(i) + dx*(c(i) + c(i) + dx*(d(i)+d(i)+d(i)))
      deriv2=2.0*(c(i) + 3.0*dx*d(i))
      deriv3=6.0*d(i)
      END   ! --------------------------------- End of Subroutine SEVAL3
